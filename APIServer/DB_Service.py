...
from sqlalchemy import (BigInteger, Column, Computed, DateTime, ForeignKey,
                        Integer, MetaData, Table, Text, and_, create_engine,
                        desc, func, select, text, update)
from sqlalchemy.dialects.postgresql import INTERVAL
from sqlalchemy.orm import Session, declarative_base, registry, relationship
from sqlalchemy.sql.functions import concat
from sqlalchemy.sql.sqltypes import NullType

from my_library import (RowsToDictList, RowToDict, append_if_not_exists,
                        delete_non_english_alphabet_characters)
...
class ReadingJournal(Base):
    ...
    id_paragraph = Column(Integer,
                          ForeignKey('sentences.id_paragraph'),
                          nullable=False)
    id_book = Column(ForeignKey('books.id_book'), nullable=False)
    dt = Column(DateTime, nullable=True)
...
class UserWordsLog(Base):
    ...
    user_id = Column(Integer, ForeignKey('users.user_id'), nullable=False)
    syllable_id = Column(Integer,
                         ForeignKey('syllables.syllable_id'),
                         nullable=False)
...
class LanguageDB:
    ...
    def Prepare_Book(self, user_name: str, book_name: str, paragraphs: list):
        ...
        for lc_paragraph in paragraphs:
            lc_result = lc_result + lc_paragraph.replace(chr(13), '').replace(
                chr(10), '').replace(chr(12), '') + chr(13)
        lc_result = lc_result.replace(chr(13) + chr(13), chr(13)).replace(
            chr(13) + chr(13),
            chr(13)).replace(chr(13) + chr(13), chr(13)).replace(
                chr(13) + chr(13),
                chr(13)).replace(chr(13) + chr(13), chr(13)).replace(
                    chr(13) + chr(13),
                    chr(13)).replace(chr(13) + chr(13), chr(13)).replace(
                        chr(13) + chr(13),
                        chr(13)).replace(chr(13) + chr(13), chr(13))
        ...
        new_book_id = self.PutBook(Book(book_name=book_name, user_id=user_id))
        first_paragraph_id = self.session.query(func.max(
            Sentence.id_paragraph)).scalar() + 1
        for paragraph in source:
            max_id_paragraph = self.session.query(
                func.max(Sentence.id_paragraph)).scalar() + 1
        ...
        book = self.session.query(Book).where(
            Book.id_book == new_book_id).first()
    ...
    def PutBook(self, pbook: Book):
        qr = self.session.query(Book).where(
            and_(Book.book_name == pbook.book_name,
                 Book.user_id == pbook.user_id))
        if qr.count() == 0:
    ...
    def PutSentence(self, pSentence: Sentence):
        qr = self.session.query(Sentence).where(
            and_(Sentence.id_book == pSentence.id_book,
                 Sentence.sentence == pSentence.sentence))
        if qr.count() == 0:
    ...
    def PutSyllabe(self, syllabe: Syllable):
        qr = self.session.query(Syllable).filter(
            Syllable.word == syllabe.word, Syllable.user_id == syllabe.user_id)
        if qr.count() == 0:
    ...
    def PutSyllablesParagraph(self, sp: SyllablesParagraph):
        qr = self.session.query(SyllablesParagraph).filter(
            SyllablesParagraph.syllable == sp.syllable,
    ...
    def GetSyllabe(self, user_id: int, word: str):
        result = self.session.query(Syllable).filter(
            Syllable.user_id == user_id, Syllable.word == word).first()
        self.IfCommit()
        return result
    ...
    def GetUserBooks(self, user_name: str):
        result = RowsToDictList(
            self.session.query(Book, User).filter(
                and_(User.name == user_name,
                     User.user_id == Book.user_id)).all())
        for i in range(len(result)):
            result[i][
                'Min_Paragraph_Number'] = self.GetMinParagraphNumberByBook(
                    user_name, result[i]['id_book'])
            result[i][
                'Max_Paragraph_Number'] = self.GetMaxParagraphNumberByBook(
                    user_name, result[i]['id_book'])
        return result
    ...
    def GetUserBookInformation(self, user_name: str, id_book: int):
        result = RowsToDictList(
            self.session.query(Book, User).filter(
                and_(User.name == user_name, User.user_id == Book.user_id,
                     Book.id_book == id_book)).first())[0]
        result['Min_Paragraph_Number'] = self.GetMinParagraphNumberByBook(
            user_name, id_book)
        result['Max_Paragraph_Number'] = self.GetMaxParagraphNumberByBook(
            user_name, id_book)
        return result

    # return sentenses of paragraph
    def GetUserBookParagraph(self, user_name: str, id_book: int,
                             id_paragraph: int):
        result = RowsToDictList(
    ...
    def GetMaxParagraphNumberByBook(self, user_name: str, id_book: int):
        result = self.session.query(Book, User, Sentence).filter(
            and_(User.name == user_name, User.user_id == Book.user_id,
                 Book.id_book == id_book,
                 Sentence.id_book == Book.id_book)).order_by(
                     desc(Sentence.id_paragraph)).first()
    ...
    def GetMinParagraphNumberByBook(self, user_name: str, id_book: int):
        result = self.session.query(Book, User, Sentence).filter(
            and_(User.name == user_name, User.user_id == Book.user_id,
                 Book.id_book == id_book,
                 Sentence.id_book == Book.id_book)).order_by(
                     Sentence.id_paragraph).first()
        return int(RowsToDictList(result)[2]['id_paragraph'])

    def GetListOfSyllables(self,
                           user_name: str,
                           ready: int,
                           slice_size: int = 100,
                           slice_number: int = 1):
        ...
        result = []
        for syllable, user in self.session.query(Syllable, User).filter(
                and_(User.name == user_name, User.user_id == Syllable.user_id,
                     Syllable.ready == ready)).order_by(
                         Syllable.last_view).offset(
                             slice_size *
                             (slice_number - 1)).limit(slice_size).all():
            result.append({
                'word': syllable.word,
    ...
    def SaveBookPosition(self, user_name: str, id_book: int,
                         new_current_paragraph: int):
        if self.GetMinParagraphNumberByBook(
                user_name, id_book
        ) <= new_current_paragraph <= self.GetMaxParagraphNumberByBook(
                user_name, id_book):
            self.session.execute(
                update(Book).where(
                    and_(Book.user_id == self.GetUserId(user_name),
                         Book.id_book == id_book)).values(
                             current_paragraph=new_current_paragraph,
                             dt=datetime.datetime.now()))
            self.session.commit()
            self.SaveBookReadingData(self.GetUserId(user_name), id_book,
            ...
            self.AddMessage(
                user_name=user_name,
                message=
                f'Page <span class="my_class_book_name_history">{book_information["book_name"]}</span> turned to <span class="my_class_a_history">{new_current_paragraph}</span>',
            ...
            result = {
                'data':
                'ERROR: New current paragraph is outside of book paragraph range.'
            }
        return result

    def GetListOfSyllablesByWordPart(self,
                                     user_name: str,
                                     ready: int,
                                     word_part: str,
                                     slice_size: int = 100,
        ...
        for syllable, user in self.session.query(Syllable, User).filter(
                and_(User.name == user_name,
                     User.user_id == Syllable.user_id, Syllable.ready == ready,
                     Syllable.word.contains(word_part))).order_by(
                         desc(Syllable.last_view)).offset(
                             slice_size *
                             (slice_number - 1)).limit(slice_size).all():
            result.append({
                'word': syllable.word,
    ...
    def GetCountOfSyllableSlices(self, user_name: str, ready: int,
                                 slice_size: int):
        result = math.ceil(
            self.session.query(Syllable, User).filter(
                and_(User.name == user_name, User.user_id == Syllable.user_id,
    ...
    def GetCountOfUserSyllables(self, user_name: str, ready: int):
        result = self.session.query(Syllable, User).filter(
            and_(User.name == user_name, User.user_id == Syllable.user_id,
    ...
    def GetCountOfUserSyllablesWorkedOutToday(self, user_name: str):
        ...
        result = self.session.query(UserWordsLog, User).filter(
            and_(
                User.name == user_name, User.user_id == UserWordsLog.user_id,
                UserWordsLog.dt >= datetime.datetime.now() -
    ...
    def GetSyllable(self, word: str, user_name: str):
        ...
        syllable, user = self.session.query(Syllable, User).filter(
            and_(User.name == user_name, User.user_id == Syllable.user_id,
                 Syllable.word == word)).first()
        ...
        result = {
            ...
            'examples':
            RowsToDictList(
                self.session.query(SyllablesParagraph).filter(
                    SyllablesParagraph.syllable_id ==
                    syllable.syllable_id).all())
        }
        self.IfCommit()
    ...
    def SetSylalbleAsViewed(self, word: str, user_name: str):
        syllable = self.session.query(Syllable).filter(
            and_(Syllable.user_id == self.GetUserId(user_name),
                 Syllable.word == word)).first()
    ...
    def GetNextSyllableForLearning(self, user_name: str):
        syllable, user = self.session.query(Syllable, User).filter(
            and_(User.name == user_name, User.user_id == Syllable.user_id,
                 Syllable.ready == 0)).order_by(Syllable.last_view).first()
        self.IfCommit()
    ...
    def SetSyllableStatus(self, word: str, user_name: str, status: int):
        ...
        self.AddMessage(
            user_name=user_name,
            message=
            f"""Word  status change <span class="messageLogTextOrange">{syllable.word}</span>""",
            icon='ready.png',
            hyperlink=f'/word_in_progress/{word}/')
        return {'data': 'ok'}
    ...
    def GetPhrases(self, user_name: str, status: int):
        qr = self.session.execute(
            select(Phrase, User).where(
                and_(User.name == user_name, Phrase.ready == status,
                     User.user_id == Phrase.user_id)).order_by(
                         Phrase.last_view)).all()
        return RowsToDictList(qr)

    def GetPhrase(self, user_name: str, phrase_id: int):
        print(f'user_name:{user_name}	  phrase_id:{phrase_id}')
        if phrase_id > 0:
            phrase, user = self.session.execute(
                select(Phrase, User).where(
                    and_(User.name == user_name, Phrase.id_phrase == phrase_id,
                         User.user_id == Phrase.user_id)).order_by(
                             Phrase.last_view)).first()
            return RowToDict(phrase)
        else:
            return {'id_phrase': 0, 'phrase': '', 'translation': ''}

    def SetPhraseStatus(self, phrase_id: int, user_name: str, status: int):
        phrase, user = self.session.query(Phrase, User).filter(
            and_(User.name == user_name, User.user_id == Phrase.user_id,
                 Phrase.id_phrase == phrase_id)).first()
        phrase.ready = status
        self.AddMessage(
            user_name=user_name,
            message=
            f"""Phrase  status change <span class="messageLogTextOrange">{phrase.phrase}</span>""",
            icon='ready.png',
            hyperlink='')
        self.session.commit()
        return {'data': 'ok'}
    ...
    def SetPhraseAsViewed(self, phrase_id: int, user_name: str):
        ...
        self.AddMessage(
            user_name=user_name,
            message=
            f"""Repeated phrase <span class="messageLogTextOrange">{phrase.phrase}</span>""",
    ...
    def GetNextPhraseForLearning(self, user_name: str):
        phrase, user = self.session.query(Phrase, User).filter(
            and_(User.name == user_name, User.user_id == Phrase.user_id,
                 Phrase.ready == 0)).order_by(Phrase.last_view).first()
        return RowToDict(phrase)
    ...
    def SavePhrase(self, user_name, phrase_id, text, translate):
        ...
        if ln_user_id > 0:
            if phrase_id > 0:
                try:
                    phrase = self.session.query(Phrase).filter(
                        and_(Phrase.id_phrase == phrase_id,
                    ...
                    self.AddMessage(
                        user_name=user_name,
                        message=
                        f"""Phrase saved <span class="messageLogTextOrange">{text}</span>""",
                ...
                try:
                    ...
                    self.AddMessage(
                        user_name=user_name,
                        message=
                        f"""Phrase added <span class="messageLogTextOrange">{text}</span>""",
                        icon='add_to_list.png',
                        hyperlink='')
                    return {"status": "ok"}
                except:
                    return {"status": "error - data has not added"}
        else:
    ...
    def SaveSyllable(self, rq):
        ...
        qr = self.session.query(Syllable).filter(
            and_(Syllable.user_id == self.GetUserId(rq.username),
                 Syllable.word == rq.word)).first()
        rq.syllable_id = (-1 if qr is None else int(
            RowToDict(qr)['syllable_id']))
        print(f'rq.syllable_id from DB:{rq.syllable_id}')
        if rq.syllable_id > 0:  #we have not zero id, so we must update syllable into DB
            syllable, user = self.session.execute(
                select(Syllable, User).where(
                    and_(
            ...
            paragraphs = self.session.query(SyllablesParagraph).filter(
                SyllablesParagraph.syllable_id == syllable.syllable_id).all()
            ...
            for example in rq.examples:  # add new examples missing into db
                if example.rowid not in rowids_db_list:
                    if len(example.example.strip()
                           ) > 0:  # only non empty examples
                        paragraph = SyllablesParagraph(
                            example=example.example,
                            translate=example.translate,
                            syllable_id=rq.syllable_id)
                        self.session.add(paragraph)
        else:  # zero syllable_id, we have to insert new data
            user_id_for_request = self.session.query(User).filter(
                User.name == rq.username).first().user_id
            ...
            print(
                f'Идентификатор вставляемой записи Syllable: {syllable.syllable_id}',
                '\n')
        ...
        self.AddMessage(
            ...
            message=
            f"""Word saved <span class="messageLogTextOrange">{rq.word}</span>""",
            icon='add.png',
            hyperlink='')
        return {'data': 'ok'}
    ...
    def GetUserId(self, user_name: str):
        user = self.session.execute(
            select(User).where(User.name == user_name)).first()

        return user[0].user_id

    # return list of learning words from book paragraphs list
    def GetListOfUserSyllableFromParagraphsId(self, user_name: str,
                                              id_book: int,
                                              paragraph_ids_list: list):
        words_list = []
        for paragraph_id in paragraph_ids_list:
            for sentence in self.GetUserBookParagraph(user_name, int(id_book),
                                                      int(paragraph_id)):
                for word in sentence['sentence'].strip().split(' '):
                    word_candidate = delete_non_english_alphabet_characters(
                        word)
                    if len(word_candidate) > 2:
                        append_if_not_exists(word_candidate.lower(), words_list)
        result = []
        for element in self.session.query(Syllable.word).filter(
                and_(Syllable.user_id == self.GetUserId(user_name),
                     Syllable.ready == 0, Syllable.word.in_(words_list))).all():
            append_if_not_exists(element[0], result)
        return result

    def GetLasReadedBookByUser(self, user_name: str):
        return self.session.query(Book.id_book).filter(
            Book.user_id == self.GetUserId(user_name)).order_by(
                Book.dt.desc()).first()[0]

    def SaveBookReadingData(self, id_user, id_book, id_paragraph):
        self.session.add(
            ReadingJournal(user_id=id_user,
                           id_book=id_book,
    ...
    def SaveTile(self, tile_id, user_name, name, hyperlink, icon, color):
        ...
        ln_user_id = self.GetUserId(user_name)
        tile = self.session.query(HPTile).filter(
            and_(HPTile.user_id == ln_user_id,
                 HPTile.tile_id == tile_id)).first()
        if tile is None:
            self.session.add(
                HPTile(user_id=ln_user_id,
                       name=name,
                       hyperlink=hyperlink,
                       icon=icon,
                       color=color))
    ...
    def GetTiles(self, user_name):
        ...
        ln_user_id = self.GetUserId(user_name)
        tiles = self.session.query(HPTile).filter(
            HPTile.user_id == ln_user_id).all()
    ...
    def GetTile(self, user_name, tile_id):
        ...
        ln_user_id = self.GetUserId(user_name)
        tile = self.session.query(HPTile).filter(
            HPTile.user_id == ln_user_id, HPTile.tile_id == tile_id).first()
        return RowToDict(tile)

    def DeleteTiles(self, user_name, tile_id):
        print(f'DeleteTiles: user_name = "{user_name}", tile_id = {tile_id}')
        ln_user_id = self.GetUserId(user_name)
        self.session.query(HPTile).filter(HPTile.user_id == ln_user_id,
                                          HPTile.tile_id == tile_id).delete()
        self.session.commit()
        return True
    ...
    def GetRows(self, user_name: str):
        ...
        ln_user_id = self.GetUserId(user_name)
        rows = self.session.query(HPRow).filter(
            HPRow.user_id == ln_user_id).order_by(HPRow.row_name).all()
        return RowsToDictList(rows)

    def GetPages(self, user_name: str):
        print(f'GetPages: user_name = "{user_name}"')
        ln_user_id = self.GetUserId(user_name)
        rows = self.session.query(HPPage).filter(
            HPPage.user_id == ln_user_id).order_by(HPPage.page_name).all()
        return RowsToDictList(rows)

    def SetPageAsDefault(self, user_name, page_id):
        print(
            f'SetPageAsDefault: user_name = "{user_name}", page_id = "{page_id}"'
        )
        ln_user_id = self.GetUserId(user_name)

        self.session.query(HPPage).filter(HPPage.user_id == ln_user_id).update(
            {'default': 0})

        self.session.query(HPPage).filter(HPPage.user_id == ln_user_id,
                                          HPPage.page_id == page_id).update(
                                              {'default': 1})
    ...
    def Delete_Row(self, user_name, row_id):
        ...
        ln_user_id = self.GetUserId(user_name)
        self.session.query(HPRowTile).filter(
    ...
    def Remove_Row_From_Page(self, user_name, page_id, row_id):
        print(
            f'Remove_Row_From_Page: user_name = "{user_name}", page_id = "{page_id}", row_id = "{row_id}"'
        )
        ln_user_id = self.GetUserId(user_name)
        self.session.query(HPPageRows).filter(
            HPPageRows.user_id == ln_user_id, HPPageRows.page_id == page_id,
    ...
    def Reorder_Index_Field_Page_Row(self, user_name, page_id):
        print(
            f'Reorder_Index_Field_Page_Row: user_name = "{user_name}", page_id = "{page_id}"'
        )
        ln_user_id = self.GetUserId(user_name)
        rows = RowsToDictList(
            self.session.query(HPPageRows).filter(
                HPPageRows.user_id == ln_user_id,
                HPPageRows.page_id == page_id).order_by(
                    HPPageRows.row_index).all())
        counter = 0
        for row in rows:
            counter += 10
            print(counter)
            self.session.query(HPPageRows).filter(
                HPPageRows.id == row['id']).update({'row_index': counter})
        self.session.commit

    def Move_In_Page_Row(self, user_name: str, direction: str, page_id: int,
                         row_id: int):
        print(
            f'Move_In_Page_Row_Up: user_name="{user_name}", direction="{direction}", page_id={page_id}, row_id={row_id}'
        )
        ln_user_id = self.GetUserId(user_name)
    ...
    def GetHPPageData(self, user_name, page_id):
        print(
            f'GetHPPageData: user_name = "{user_name}", page_id = "{page_id}"')
        ln_user_id = self.GetUserId(user_name)

        # the page - one record by parameter page_id
        page = self.session.query(HPPage).filter(
            HPPage.user_id == ln_user_id, HPPage.page_id == page_id).first()
    ...
    def GetHPRowData(self, user_name: str, row_id):
        ...
        ln_user_id = self.GetUserId(user_name)
        row = RowToDict(
            self.session.query(HPRow).filter(
                HPRow.user_id == ln_user_id,
                HPRow.row_id == int(row_id)).first())
        tiles = RowsToDictList(
            self.session.query(HPTile, HPRowTile).filter(
                HPRow.row_id == int(
                    row_id),  # HPRow.row_id == HPRowTile.row_id,
    ...
    def AddRowIntoPage(self, user_name: str, page_id, row_id: int):
        print(
            f'AddRowIntoPage: user_name = "{user_name}", page_id = "{page_id}", row_id = "{row_id}"'
        )
        ln_user_id = self.GetUserId(user_name)
    ...
    def AddTileToRowRelation(self, user_name, row_id, tile_id, index_id):
        print(
            f'AddTileToRowRelation: user_name = "{user_name}", row_id = "{row_id}", tile_id = {tile_id}, index_id = {index_id}'
        )
        ln_user_id = self.GetUserId(user_name)
        queru_exists = self.session.query(HPRowTile).filter(
            HPRowTile.user_id == ln_user_id, HPRowTile.row_id == row_id,
    ...
    def SaveRowName(self, user_name, row_id, new_row_name):
        print(
            f'SaveRowName: user_name = "{user_name}", row_id = "{row_id}", new_row_name = "{new_row_name}"'
        )
        ln_user_id = self.GetUserId(user_name)
        if row_id == 0:
            ...
            updated_row = self.session.query(HPRow).filter(
                HPRow.user_id == ln_user_id, HPRow.row_id == row_id).first()
            updated_row.update({'row_name': new_row_name})
        self.session.commit()
        print(
            f'    {"add new" if row_id==0 else "update "} row {updated_row.row_id}'
        )
    ...
    def AddMessage(self, user_name, message='', icon='', hyperlink=''):
        ...
        result = self.loop.call_soon_threadsafe(asyncio.ensure_future,
                                                self.SendRequest(url, message))
        return {"status": "ok - added"}
