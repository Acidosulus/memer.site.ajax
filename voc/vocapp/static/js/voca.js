...
async function FillHomePage() {
  let pattern = `
    <div class="container mt-5" style="border: 2px solid #cccccc; border-radius: 15px;" id="page_rows_in_edit_form" data-row_id="{ row_id }">
      <div class="row m-1" id="row_container">
`;
  for (let i = 1; i <= 12; i++){
    pattern += `
  ...
  if (page_id>0){
    ...
    for (let row of response.rows){
      ...
      for (let tile of row.tiles){
        ...
        rowHtml = rowHtml.replaceAll(  `{ hyperlink_${tile.tile_index} }`,
                                    tile.hyperlink);
